...
class BuyerFarmerProfileViewScreen extends StatefulWidget {
  final int farmerId;
  final String farmerName;

  const BuyerFarmerProfileViewScreen({
    super.key,
  ...
  @override
  State<BuyerFarmerProfileViewScreen> createState() => _BuyerFarmerProfileViewScreenState();
}

class _BuyerFarmerProfileViewScreenState extends State<BuyerFarmerProfileViewScreen> {
  ...
  Future<void> _fetchProfile() async {
    ...
    try {
      final res = await ApiService.getFarmerProfilePublic(widget.farmerId);
      if (mounted) {
        if (res['success'] == true) {
          setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    final farmerVerification = _profileData['farmer_verification'] ?? {};
    final List<dynamic> reviews = _profileData['reviews'] ?? [];
    final avgRating = _profileData['avg_rating'] ?? 0.0;
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('${widget.farmerName}\'s Profile', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
        elevation: 0,
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        child: Column(
                          children: [
                            ...
                            const SizedBox(height: 16),
                            Text(
                              user['full_name'] ?? widget.farmerName,
                            ...
                            if (user['phone_number'] != null) ...[
                              ...
                              SizedBox(
                                ...
                                child: ElevatedButton.icon(
                                  ...
                                  label: const Text('Call Farmer', style: TextStyle(fontSize: 13, color: Colors.white, fontWeight: FontWeight.bold)),
                      ...
                      Container(
                        ...
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            const Text('Verification & Certifications', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Color(0xFF0F172A))),
                            const SizedBox(height: 16),
                            _buildInfoRow(Icons.description_rounded, 'Farming License', farmerVerification['farming_license_number'] ?? 'Not Verified'),
                            _buildInfoRow(Icons.stars_rounded, 'Organic Cert No.', farmerVerification['organic_certificate_number'] ?? 'No Certificate'),
                            _buildInfoRow(Icons.workspace_premium_rounded, 'GAP Cert No.', farmerVerification['gap_certificate_number'] ?? 'No GAP Certificate'),
                            _buildInfoRow(Icons.landscape_rounded, 'Cultivated Lands', '${farmerVerification['total_lands'] ?? 1} Registered Lands'),
                          ],
                        ),
                      ),
                      const SizedBox(height: 20),
                      ...
                      if (reviews.isEmpty)
                        Container(
                          ...
                          child: Column(
                            children: [
                              ...
                              Text('No reviews yet for this farmer.', style: TextStyle(color: Colors.grey[500], fontSize: 12)),
                            ],
                          ),
                        )
                      else
                        ListView.separated(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final reviewerName = review['reviewer_name']?.toString() ?? 'Aswenna Buyer';
                                  ...
                                  Row(
                                    children: [
                                      CircleAvatar(
                                        ...
                                        child: photoUrl == null
                                            ? Text(
                                                reviewerName.isNotEmpty ? reviewerName[0].toUpperCase() : 'B',
