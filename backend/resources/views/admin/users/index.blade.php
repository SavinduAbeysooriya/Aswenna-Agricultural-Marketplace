...
<head>
    ...
    <title>Aswenna - Users List ({{ ucwords(str_replace('_', ' ', $role)) }}s)</title>
    ...
    <div class="flex w-full min-h-screen">
        ...
        <div class="flex-1 flex flex-col min-w-0 min-h-screen">
            ...
            <main class="flex-1 p-4 sm:p-6 md:p-8 overflow-y-auto w-full max-w-[1700px] mx-auto">
                <livewire:admin.users-table :role="$role" />
            </main>
        ...
        document.addEventListener('livewire:init', () => {
            Livewire.on('user-saved', (event) => {
                Swal.fire({
                    icon: 'success',
                    title: 'Success',
                    text: event.message || 'User status updated.',
                    timer: 1800,
            ...
            Livewire.on('user-failed', (event) => {
                Swal.fire({
                    icon: 'error',
                    title: 'Action Failed',
                    text: event.message || 'There was an issue processing your request.',
                    customClass: {
                        popup: 'rounded-3xl shadow-2xl border border-slate-100'
                    }
                });
