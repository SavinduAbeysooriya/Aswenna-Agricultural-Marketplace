...
<head>
    ...
    <title>Aswenna - Offer Campaigns & Goals</title>
...
<body class="min-h-screen bg-[#F8FAFC] text-slate-800 antialiased selection:bg-emerald-500/30">
    ...
    <div class="flex w-full min-h-screen">
        ...
        <div class="flex-1 flex flex-col min-w-0 min-h-screen">
            ...
            <main class="flex-1 p-4 sm:p-6 md:p-8 overflow-y-auto w-full max-w-[1700px] mx-auto">
                <livewire:admin.offer-campaigns-table />
            </main>
    ...
    <script>
        ...
        document.addEventListener('livewire:init', () => {
            Livewire.on('campaign-saved', (event) => {
                Swal.fire({
                    icon: 'success',
                    title: 'Saved',
                    text: event.message || 'Offer campaign saved successfully.',
                    timer: 1800,
            ...
            Livewire.on('goal-saved', (event) => {
                Swal.fire({
                    icon: 'success',
                    title: 'Saved',
                    text: event.message || 'Offer goal saved successfully.',
                    timer: 1800,
                    showConfirmButton: false,
                    customClass: {
                        popup: 'rounded-3xl shadow-2xl border border-slate-100'
            ...
            window.addEventListener('confirm-campaign-delete', (event) => {
                ...
                Swal.fire({
                    ...
                    text: `This permanently removes the campaign "${event.detail.title || 'this campaign'}".`,
                    ...
                    if (result.isConfirmed && component) {
                        component.call('deleteCampaign', event.detail.campaignId);
                    }
                });
            });

            window.addEventListener('confirm-goal-delete', (event) => {
                const componentRoot = event.target?.closest?.('[wire\\:id]');
                const component = componentRoot ? Livewire.find(componentRoot.getAttribute('wire:id')) : null;

                Swal.fire({
                    icon: 'warning',
                    title: 'Remove offer goal?',
                    text: `This permanently removes "${event.detail.name || 'this goal'}" and its associated campaigns.`,
                    showCancelButton: true,
                    confirmButtonColor: '#e11d48',
                    cancelButtonColor: '#94a3b8',
                    confirmButtonText: 'Yes, remove',
                    ...
                    if (result.isConfirmed && component) {
                        component.call('deleteGoal', event.detail.goalId);
                    }
                });
            });
        });
